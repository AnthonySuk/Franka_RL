...
class FrankaOnlyRobot:
    ...
    def control_ik(self,local_ee_pose,local_goal_pose,local_j_eef):
        ...
        lmbda = torch.eye(7, device=self.device) * (0.05 ** 2)
        u = (j_eef_T @ torch.inverse(local_j_eef @ j_eef_T + lmbda) @ dpose).view(self.num_envs, 7)
    ...
    def step(self, actions):
        ...
        # print("target dof pos :", self.arm_u)s
    ...
    def _compose_proprioceptive_obs_buf_no_height_measure(self):
        ...
        self.proprioceptive_obs_buf = torch.cat((
                                             ...
                                             self.actions, # 7
                                             self.curr_ee_goal_local,# 3
                                             self.ee_pos_local,# 3
                                             self.ee_orn, # 3
                                             self.ee_goal_orn_quat, # 4
                                             self.ee_pos_local - self.curr_ee_goal_local # 4
                                             ), dim=-1)
    ...
    def _init_buffers(self):
        ...
        print("Init-num_dof:",self.num_dof)
        print("Init-dof_pos:",self.dof_pos)
