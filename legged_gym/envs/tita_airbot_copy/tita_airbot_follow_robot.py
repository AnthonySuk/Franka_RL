...
from legged_gym.utils.math import quat_apply_yaw, wrap_to_pi, euler_from_quat, torch_wrap_to_pi_minuspi
from legged_gym.utils.terrain import Terrain
from collections import deque
import numpy as np

class TitaAirbotRobot_F:
    ...
    def step(self, actions):
        ...
        arm_pos_targets = self.control_ik(dpose) + self.dof_pos[:, self.arm_indices]
        all_pos_targets = torch.zeros_like(self.dof_pos)
        all_pos_targets[:, self.arm_indices] = arm_pos_targets
        #all_pos_targets[:, self.arm_indices] = 0 # default angle

        for _ in range(self.cfg.control.decimation):
            self.torques = self._compute_torques(self.actions) #.view(self.torques.shape)
            #print("calculated torque :{}".format(self.torques))
    ...
    def post_physics_step(self):
        ...
        #print("dof_torque",self.dof_torque)
        ...
        masked_vel = (torch.abs(thigh_vel) > 2.).float()

        self.dof_vel_history.append(thigh_vel.clone() * masked_vel)
        if self.episode_length_buf % 50 == 0 and len(self.dof_vel_history) >= 20:
            is_oscillating = self.check_oscillation(self.dof_vel_history)
            for env_id in range(self.num_envs):
                for i, dof_idx in enumerate(range(2)):
                    if is_oscillating[env_id, i]:
                        print(f"[Env {env_id}] DOF {dof_idx} is oscillating!")
    ...
    def check_oscillation(self,vel_history, sign_thresh=30):
        # pos_history: deque of (num_envs, 2) tensors
    ...
    def _compose_privileged_obs_buf_no_height_measure(self):
        ...
        self.dof_err[:,self.arm_indices] = 0
    ...
    def _compute_torques(self, actions):
        ...
        # define action
        actions_tita = actions[:,:8] # first 8
        actions_arm = actions[:,8:] # last 6
        # define pos and vel
        modify_dof_vel = self.dof_vel.clone().detach()
        modify_dof_pos = self.dof_pos.clone().detach()
        modify_dof_pos[:, 8:] = torch_wrap_to_pi_minuspi(modify_dof_pos[:, 8:])
        # define control type
        control_type = self.cfg.control.control_type

        if control_type == "P_AND_V":
            # torque for tita
            dof_err_tita = action_scale_pos * actions_tita + self.default_dof_pos[:,:8] - modify_dof_pos[:,:8]
            torques_tita = self.p_gains[:8] * (dof_err_tita) - self.d_gains[:8] * modify_dof_vel[:,:8]
            # Choose joints to V control. We only care about wheels velocity, so velocity control mode is used.
            V_list = [3,7]
            torques_tita[:,V_list] = self.d_gains[V_list]*(action_scale_vel* actions_tita[:,V_list] - modify_dof_vel[:,V_list])
            # torque for arm
            dof_err_arm = action_scale_arm * actions_arm + self.default_dof_pos[:,8:] - modify_dof_pos[:,8:]
            torques_arm = self.p_gains[8:] * (dof_err_arm) - self.d_gains[8:] * modify_dof_vel[:,8:]
            # tital torque
            torques = torch.cat([torques_tita, torques_arm], dim=-1)
        else:
    ...
    def _get_noise_scale_vec(self):
        ...
        obs_noise_vec[9:command_end_idx] = 0.  
        # dof_pos dimension: 14
        ...
        # action dimension: 8 (mask out arm)
        last_action_end_idx = dof_vel_end_idx + self.num_actions - 6
    ...
    def _init_buffers(self):
        ...
        self.dof_vel_history = deque(maxlen=50)
        ...
        for i in range(self.num_dofs):
            ...
            print("###Current joint ",name," P: ",self.p_gains[i]," D: ",self.d_gains[i]," Default angle: ",self.default_dof_pos[i])
    ...
    def _create_envs(self):
        ...
        for j in range(self.num_envs):
            ...
            # print("### Dof_mode: ",dof_props["driveMode"])  # This is a numpy array of drive modes per DOF
...
# ee goal function
    ...
    def _reward_dof_pos_limits(self):
        ...
        out_of_limits[:,:8] = 0 # Only penalize arm limit
    ...
    def _reward_tracking_ee_sphere(self):
        ee_pos_local = quat_rotate_inverse(self.base_yaw_quat, self.ee_pos - self._get_ee_goal_spherical_center())
        ee_pos_error = torch.sum(torch.abs(self.cart2sphere(ee_pos_local) - self.curr_ee_goal_sphere) * self.sphere_error_scale, dim=1)
    ...
    def _reward_tracking_ee_cart(self):
        target_ee = self._get_ee_goal_spherical_center() + quat_apply(self.base_yaw_quat, self.curr_ee_goal_cart)
